...
import os
from mt_utils import mt_utils

tf.app.flags.DEFINE_integer("mini_batch_size", 10000,
                            """mini batch size.""")
...
tf.app.flags.DEFINE_boolean("set_profiling", False,
                            ...
                            """TensorBoard).""")
tf.app.flags.DEFINE_boolean("data_freeze", False,
                            """freeze the data while training the optimzier.""")
...
# MODIFY
tf.app.flags.DEFINE_boolean("if_mt", False, "")
tf.app.flags.DEFINE_float("mt_ratio", 0.1, "")
tf.app.flags.DEFINE_integer("mt_k", 1, "")
...
def train_optimizer(logdir,
  ...
  if if_cl:
    ...
    num_unrolls_train = [int(ns / fix_unroll_length) for ns in num_steps]
    num_unrolls_eval = num_unrolls_train[1:]
    min_num_eval = 3
    curriculum_idx = 0
    num_eval = 0
    improved = False
  ...
  for problem_itr, (problem_spec, dataset, batch_size) in enumerate(sampler):
    ...
    with graph.as_default(), graph.device(device_setter):
      ...
      # MODIFY
      def get_meta_grad(obj):
          grads_and_vars = meta_opt.compute_gradients(obj + reg_l2,
                                                      meta_parameters)
      ...
      clipped_grads_and_vars = get_meta_grad(train_output.metaobj)
      # builds the train and summary operations
      train_op = meta_opt.apply_gradients(clipped_grads_and_vars,
                                          global_step=global_step)
      if FLAGS.if_mt:
        clipped_grads_and_vars_mt = get_meta_grad(train_output.metaobjmt)
        train_op_mt = meta_opt.apply_gradients(clipped_grads_and_vars_mt,
                                               global_step=global_step)
      ...
      with sv.managed_session(config=config) as sess:
        ...
        for k in range(num_meta_iterations):
          ...
          # MODIFY
          init_tensors = [normal(size=shape) for shape in problem.param_shapes]
          ...
          # MODIFY
          if if_cl:
            num_unrolls = num_unrolls_train[curriculum_idx]
            partial_unroll_iters = [fix_unroll_length for _ in xrange(num_unrolls)]
          elif not fix_unroll:
          ...
          print ("num_unrolls:")
          print (num_unrolls)
          print ("partial_unroll_iters:")
          print (partial_unroll_iters)
          print ("total_num_iter:")
          print (total_num_iter)
          ...
          # MODIFY
          if mt_training:
            db = datasets.batch_indices(mini_batch_size, total_num_iter*FLAGS.mt_k, batch_size)
            dataset_batches = []
            last_index = 0
            for num in partial_unroll_iters:
              dataset_batches.append(db[last_index: last_index + num*FLAGS.mt_k: FLAGS.mt_k ])
              last_index += num*FLAGS.mt_k
            dataset_batches_mt = []
            last_index = 0
            for num in partial_unroll_iters:
              dataset_batches_mt.append(db[last_index: last_index + num*FLAGS.mt_k])
              last_index += num*FLAGS.mt_k
          else:
          ...
          # MODIFY
          if FLAGS.data_freeze:
            print('data freeze')
            mini_batches = [np.arange(mini_batch_size)] * num_unrolls
          else:
            print('use full dataset')
            full_batch = np.arange(data.shape[0])
            mini_batches = []  # (num_unrolls, mini_batch_size)
            for _ in range(num_unrolls):
              np.random.shuffle(full_batch)
              mini_batches.append(full_batch[: mini_batch_size])

          # get mt labels
          # (num_unrolls, unroll_len, num_params, param_shape)
          if mt_training:
            mt_labels = _mt_utils.get_mt_labels(init_tensors,
                                                data, labels,
                                                mini_batches, dataset_batches_mt, k=FLAGS.mt_k)
          else:
            mt_labels = [[[np.zeros(shape=pshape) for pshape in problem.param_shapes]] * unroll_len
                         for unroll_len in partial_unroll_iters]
          ...
          for unroll_itr in range(num_unrolls):
            ...
            if FLAGS.regularize_time == 'posterior':
              ...
              if unroll_itr < int(num_unrolls * FLAGS.reg_scale + 1):
                ...
                jacob_switch = False
            elif FLAGS.regularize_time == 'none':
              jacob_switch = False
            else:
              jacob_switch = True
            ...
            # mini_batch
            # pdb.set_trace()
            mini_data, mini_batch = None, None
            # MODIFY
            # if FLAGS.data_freeze:
            #   # only use part of the entire datasedt
            #   print('data freeze')
            #   mini_batch = np.arange(mini_batch_size)
            # else:
            ...
            # mini_data = data[: mini_batch_szie]
            # mini_labels = labels[: mini_batch_size]
            ...
            # MODIFY
            feed[train_output.mode_mt] = mt_training
            num_params = len(mt_labels[0][0])
            # (num_params, unroll_length, param_shape)
            mt_labels_reshape = [[label_unroll_itr[pi] for label_unroll_itr in mt_labels[unroll_itr]]
                                 for pi in range(num_params)]
            for pi, mt_label_pl in enumerate(train_output.mt_labels):
              feed[mt_label_pl] = np.array(mt_labels_reshape[pi])
            for ti in range(len(init_tensors)):
              feed[train_output.init_tensors[ti]] = init_tensors[ti]

            if mt_training:
              fetches_list = [train_op_mt]
              sess.run(fetches_list, feed_dict=feed,
                       options=run_options, run_metadata=run_metadata)
              if unroll_itr + 1 < num_unrolls:
                sess.run(propagate_loop_state_op, feed_dict=feed,
                         options=run_options, run_metadata=run_metadata)
              continue
            # print('系统已经使用内存:%d.3GB' % (float(mem.used) / 1024 / 1024 / 1024))
            # print('系统空闲内存:%d.3GB' % (float(mem.free) / 1024 / 1024 / 1024))
          ...
          if (k + 1) % evaluation_period == 0:
            ...
            for _ in range(evaluation_epochs):
              if if_cl:
                num_unrolls_val = num_unrolls_eval[curriculum_idx]
                partial_unroll_iters_val = [fix_unroll_length for _ in xrange(num_unrolls_val)]
                total_num_iter_val = sum(partial_unroll_iters_val)
              ...
              mt_labels = [[[np.zeros(shape=pshape) for pshape in problem.param_shapes]] * unroll_len
                           for unroll_len in partial_unroll_iters_val]
              num_params = len(mt_labels[0][0])
              # (num_params, unroll_length, param_shape)
              mt_labels_reshape = [[label_unroll_itr[pi] for label_unroll_itr in mt_labels[unroll_itr]]
                                   for pi in range(num_params)]
              init_tensors = [normal(size=shape) for shape in problem.param_shapes]
              cost = validate(sess, k, num_unrolls_val, partial_unroll_iters_val, total_num_iter_val, batch_size,
                              mini_data, mini_labels, train_output,
                              objective_weights_val, dataset, propagate_loop_state_op,
                              run_options, run_metadata, jacob_switch, init_tensors, mt_labels_reshape)
              cost_total += cost
            cost = cost_total / evaluation_epochs

            if if_cl:
              print ("evalution {}, num_steps={}, cost={}".format(
                (k + 1) // evaluation_period, num_steps[curriculum_idx], cost), flush=True)
              num_eval = num_eval + 1
              if cost < best_evaluation:
                best_evaluation = cost
                improved = True
                # save model
                sv.saver.save(sess, sv.save_path + "-{}".format(curriculum_idx))
                sv.saver.save(sess, sv.save_path + "-{}".format(0))

              elif num_eval >= min_num_eval and improved:
                # restore model
                ckpt = os.path.join(logdir, "model.ckpt-{}".format(curriculum_idx))
              ...
              print ("evalution {}, cost={}".format((k + 1) // evaluation_period, cost), flush=True)
              if cost < best_evaluation:
                best_evaluation = cost
                sv.saver.save(sess, sv.save_path + "-{}".format(0))
                best_model_idx = k + 1

          if not if_cl and (k+1) % save_period == 0:
            sv.saver.save(sess, sv.save_path + "-{}".format(k + 1))
    ...
    """
    if not if_cl:
      print ("best model index={}".format(best_model_idx))


# MODIFY
def validate(sess, k, num_unrolls, partial_unroll_iters, total_num_iter, batch_size, mini_data, mini_labels, train_output, objective_weights, dataset, propagate_loop_state_op, run_options, run_metadata, jacob_switch, init_tensors, mt_labels):
  print ("validation...")
  for unroll_itr in range(num_unrolls):
    first_unroll = unroll_itr == 0
    if FLAGS.reset_rnn_params:
    ...
    db = datasets.batch_indices(FLAGS.mini_batch_size, total_num_iter, batch_size)
    dataset_batches = []
    last_index = 0
    for num in partial_unroll_iters:
      dataset_batches.append(db[last_index:last_index + num])
    ...
    for ti in range(len(init_tensors)):
      feed[train_output.init_tensors[ti]] = init_tensors[ti]
    for pi, mt_label_pl in enumerate(train_output.mt_labels):
      feed[mt_label_pl] = np.array(mt_labels[pi])
    fetches_list = [
      train_output.metaobj,
      train_output.problem_objectives
    ]
