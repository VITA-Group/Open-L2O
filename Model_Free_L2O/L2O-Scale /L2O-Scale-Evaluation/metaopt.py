        ...
        for k in range(num_meta_iterations):
          ...
          if (k+1) % evaluation_period == 0:
            ...
            for _ in range(evaluation_epochs):
              ...
              cost = validate(sess, k, num_unrolls_val, partial_unroll_iters_val, total_num_iter_val, batch_size, train_output,
                       objective_weights_val, dataset, propagate_loop_state_op, run_options, run_metadata)
              cost_total += cost
            ...
            if if_cl:
              ...
              if cost < best_evaluation:
                best_evaluation = cost
                sv.saver.save(sess, sv.save_path + "-iter{}".format(0))
                best_model_idx = k+1

          if not if_cl and (k+1) % save_period == 0:
            sv.saver.save(sess, sv.save_path + "-iter{}".format(k + 1))
...
# MODIFY
def validate(sess, k, num_unrolls, partial_unroll_iters, total_num_iter, batch_size, train_output, objective_weights, dataset, propagate_loop_state_op, run_options, run_metadata):
  for unroll_itr in range(num_unrolls):
    first_unroll = unroll_itr == 0
    if FLAGS.reset_rnn_params:
      reset_state = first_unroll and k == 0
...
def test_optimizer(optimizer,
  ...
  with tf.Session(graph=graph) as sess:
    ...
    if logdir is not None:
      ...
      ckpt = os.path.join(logdir, restore_model_name)
      restorer.restore(sess, ckpt)
    else:
