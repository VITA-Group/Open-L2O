...
class ConvNetOrig(SoftmaxClassifier):
  ...
  def __init__(self,
    ...
    super(ConvNetOrig, self).__init__(param_shapes, random_seed, noise_stdev)
...
class ConvNet(SoftmaxClassifier):
  ...
  def __init__(self,
    ...
    # Number of channels, number of pixels in x- and y- dimensions.
    self.mode = mode
    if len(filter_list[0]) > 3:
      self.stride = filter_list[0][3]
    else:
      self.stride = 1
    n_channels, px, py = image_shape
  ...
  def inference(self, params, data):
    ...
    for i in range(0, len(w_conv_list), 2):
      ...
      layer = tf.nn.conv2d(conv_input, w_conv, strides=[1, self.stride, self.stride, 1], padding="VALID")
      layer = tf.nn.bias_add(layer, b_conv)
      layer = tf.layers.batch_normalization(layer, training=training)
      layer = self.activation(layer)
      output = tf.nn.max_pool(layer, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="VALID")
      conv_input = output
    # Flatten.
    flattened = tf.reshape(conv_input, (128, self.affine_size))
