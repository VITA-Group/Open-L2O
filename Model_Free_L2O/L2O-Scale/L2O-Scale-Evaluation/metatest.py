...
tf.app.flags.DEFINE_string("save_dir", "opt/",
                           """Directory to testing results.""")
...
tf.app.flags.DEFINE_boolean("include_mnist_conv_orig_problems", False,
                            """Include Convnet problems.""")
# MODIFY
tf.app.flags.DEFINE_boolean("include_mnist_conv_wider_problems", False,
                            """Include Convnet problems.""")

tf.app.flags.DEFINE_boolean("include_cifar10_conv_problems", False,
                            """Include cifar10 Convnet problems.""")
tf.app.flags.DEFINE_boolean("include_mnist_conv_4layers_problems", False,
                            """Include Convnet problems.""")
tf.app.flags.DEFINE_boolean("include_cifar10_conv_4layers_problems", False,
                            """Include cifar10 Convnet problems.""")
...
tf.app.flags.DEFINE_boolean("include_mnist_mlp_problems", False,
                            """Include Convnet problems.""")
tf.app.flags.DEFINE_boolean("include_mnist_mlp_relu_problems", False,
                            """Include Convnet problems.""")
tf.app.flags.DEFINE_boolean("include_mnist_mlp_deeper_problems", False,
                            """Include Convnet problems.""")
...
# MODIFY
tf.app.flags.DEFINE_string("model_name", "mt", "")
tf.app.flags.DEFINE_string("restore_model_name", "", "")
...
def main(_):
    ...
    # MODIFY
    """
    if FLAGS.include_mnist_conv_wider_problems:
        problems_and_data.extend(ps.test_mnist_conv_wider_problems())
    
    if FLAGS.include_cifar10_conv_problems:
        problems_and_data.extend(ps.test_cifar10_conv_problems())
        
    if FLAGS.include_mnist_conv_4layers_problems:
        problems_and_data.extend(ps.test_mnist_conv_4layers_problems())

    if FLAGS.include_cifar10_conv_4layers_problems:
        problems_and_data.extend(ps.test_cifar10_conv_4layers_problems())
    ...
    for problem_itr, (problem, dataset, batch_size) in enumerate(problems_and_data):
        ...
        # problem_feature = FLAGS.train_dir.split('/')[1][3:]
        # MODIFY
        ...
        for seed in [6, 12, 18, 24, 30]:
            ...
            objective_values, _, _ = metaopt.test_optimizer(
                ...
                logdir=logdir,
                record_every=None,
                restore_model_name=FLAGS.restore_model_name)  # MODIFY

            # MODIFY
            with open(os.path.join(save_dir, 'seed{}_eval_loss_record.pickle-{}'.format(seed, FLAGS.model_name)), 'wb') as l1_record:
