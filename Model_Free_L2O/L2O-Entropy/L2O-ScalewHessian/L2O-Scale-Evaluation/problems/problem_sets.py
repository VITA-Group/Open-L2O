...
def test_mnist_mlp_deeper_problems():
  tp = [
      (_Spec(pg.FullyConnected, (784, 10),
             {"hidden_sizes": (20, 20), "activation": tf.nn.sigmoid}),
       datasets.mnist(train=False), 128)
  ]
  return tp
...
def test_mnist_conv_problems():
  return [
      (_Spec(pg.ConvNet, ((1, 28, 28), 10, [(3, 3, 16, 1), (5, 5, 32, 1)]),
             {"activation": tf.nn.relu, "affine_size": 512}), datasets.mnist(train=False), 128)
  ]

def test_mnist_conv_orig_problems():
    return [
        (_Spec(pg.ConvNetOrig, ((1, 28, 28), 10, [(3, 3, 8), (5, 5, 8)]),
              {"activation": tf.nn.relu}), datasets.mnist(train=False), 64)
    ]

def test_cifar10_conv_problems():
  return [
      (_Spec(pg.ConvNet, ((3, 32, 32), 10, [(3, 3, 16, 2), (5, 5, 32, 2)]),
             {"activation": tf.nn.relu, "affine_size": 32}), datasets.cifar10(train=False), 128)
  ]
